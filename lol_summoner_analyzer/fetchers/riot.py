...
class RiotClient:
    ...
    def _get(self, url: str, params: dict | None = None, retries: int = 3) -> dict:
        ...
        Retries up to `retries` times on 429, respecting Retry-After when present.
        """
        for attempt in range(retries):
            try:
                ...
                if attempt == retries - 1:
                    raise RioteApiError(0, "Request timed out after 10s")
        ...
        raise RioteApiError(429, "Rate limit: exhausted all retry attempts")
    ...
    def get_timeline(self, match_id: str) -> dict:
        """Return the per-minute timeline for a match (gold, CS, XP diffs)."""
        url = (
            f"https://{self.routing}.api.riotgames.com"
